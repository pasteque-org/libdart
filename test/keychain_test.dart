...
void main() {
  group('keychain to DID', () {
    test('should encode the key material metadata', () {
      final seed = Uint8List.fromList(
        utf8.encode('abcdefghijklmnopqrstuvwxyz'),
      );
      ...
      final address = crypto.deriveAddress(
        utf8.decode(seed),
        0,
        isSeedHexa: false,
      );
  ...
  group('derivation', () {
    test('should derive keys for a given service with suffix', () {
      ...
      final keyPair = keychain.deriveKeypair('uco');
      final extendedKeyPair = keychain.deriveKeypair(
        'uco',
        pathSuffix: 'extended',
      );
    ...
    test('should derive key with derivation path without index', () {
      ...
      final keychainWithIndex = Keychain(
        seed: seed,
      ).copyWithService('uco', "m/650'/0/0");
      final keyPairWithIndex = keychainWithIndex.deriveKeypair('uco');

      final keychainWithoutIndex = Keychain(
        seed: seed,
      ).copyWithService('uco', "m/650'/0");
      ...
      final hashedPath = sha256.process(
        Uint8List.fromList(utf8.encode("m/650'/0")),
      );
      ...
      final normalDerivationKeyPair = crypto.deriveKeyPair(
        uint8ListToHex(serviceSeed),
        0,
      );
  ...
  group('buildTransaction', tags: <String>[TestTags.integration], () {
    test('should add service in a keychain', () async {
      ...
      final keychainTransaction = apiService.newKeychainTransaction(
        keychainSeed,
        <String>[uint8ListToHex(Uint8List.fromList(walletKeyPair.publicKey!))],
        Uint8List.fromList(hexToUint8List(originPrivateKey)),
        blockchainTxVersion,
        servicesMap: {kServiceName: kDerivationPath},
      );
      dev.log('keychainTransaction: ${keychainTransaction.toNodeRPC()}');
      ...
      ).send(transaction: keychainTransaction);

      await ArchethicTransactionSender(
        apiService: apiService,
      ).send(transaction: accessKeychainTx);
      ...
      final genesisAddressKeychain = crypto.deriveAddress(
      ...
      keychainToUpdate = keychainToUpdate.copyWithService(
        kServiceName2,
        kDerivationPath2,
      );
      ...
      final keychainTransaction2 = Transaction(
        type: 'keychain',
        ...
        authorizedKeys.add(
          AuthorizedKey(
            encryptedSecretKey: uint8ListToHex(
              crypto.ecEncrypt(aesKey, ak.publicKey),
            ),
      ...
      await ArchethicTransactionSender(apiService: apiService).send(
    ...
    test(
      ...
      () async {
        ...
        final keychainTransaction = apiService.newKeychainTransaction(
          keychainSeed,
          <String>[
            uint8ListToHex(Uint8List.fromList(walletKeyPair.publicKey!)),
          ],
          Uint8List.fromList(hexToUint8List(originPrivateKey)),
          blockchainTxVersion,
          servicesMap: {kServiceName: kDerivationPath},
