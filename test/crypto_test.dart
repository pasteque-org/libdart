...
void main() {
  group('Crypto', () {
    group('hash', () {
      ...
      test(
        'should generate a sha3-256 hash with an algo id at the begining',
        () {
          expect(
            uint8ListToHex(
              crypto.hash('myfakedata', algo: 'sha3-256', isContentHexa: false),
            ),
      ...
      test(
        'should generate a sha3-512 hash with an algo id at the beginning',
        () {
          expect(
            uint8ListToHex(
              crypto.hash('myfakedata', algo: 'sha3-512', isContentHexa: false),
            ),
    ...
    group('ecEncrypt', () {
      test('should encrypt a data using a ed25519 public key', () {
        final keypair = crypto.deriveKeyPair('seed', 0, isSeedHexa: false);
        final cipherText = crypto.ecEncrypt(
          'hello',
          keypair.publicKey,
          isDataHexa: false,
        );
  ...
  group('Address', () {
    ...
    group('deriveKeyPair', () {
      ...
      test(
        'should derive a keypair by using a seed and index using secp256k1',
        () {
          expect(
            uint8ListToHex(
              Uint8List.fromList(
