...
void main() {
  group('api', tags: <String>[TestTags.integration], () {
    test('getTransactionIndex', () async {
      final transaction = await ApiService(
        'https://mainnet.archethic.net',
      ).getTransactionIndex([
        '000022372EFF37915F6CAE040A4DE3C8BB37863CB9531F314F404CB61B6C17CB7CD6',
      ]);
      expect(
        transaction['000022372EFF37915F6CAE040A4DE3C8BB37863CB9531F314F404CB61B6C17CB7CD6'],
        1,
      );
    });
    ...
    test('fetchBalance', () async {
      final balanceMap = await ApiService(
        'https://testnet.archethic.net',
      ).fetchBalance([
      ...
      expect(
        balanceMap['0000ce09251f97c629cba7940e7a860488d60c02ddb47eaa43df30e37f06b9e34f9d']!
            .uco,
        10000000000,
      );
    });
    ...
    test('getTransactionContent', () async {
      final contentMap = await ApiService(
        'https://testnet.archethic.net',
      ...
      expect(
        contentMap['0000C85F189C13846ED91FFE30B08EF3B0E009780DAEEF4581F8B3D717D99CC954C0'],
    ...
    test('getTransactions with criteria', () async {
      ...
      expect(
        transactionChainListMap.values.first[0].address!.address!.toUpperCase(),
    ...
    test('getNodeList', () async {
      ...
      expect(nodes[0].ip, '188.166.234.20');
    });

    test('networkTransactions', () async {
      final transactionsList = await ApiService(
        'https://testnet.archethic.net',
      ).networkTransactions('oracle_summary', 1);

      expect(transactionsList[0].type, 'oracle_summary');
    });

    test('transactionInputs', () async {
      final transactionInputsList = await ApiService(
        'https://mainnet.archethic.net',
    ...
    test('getTransactionFees', () async {
      ...
      expect(transactionFee.fee, greaterThanOrEqualTo(0));
    });
    ...
    test('getToken', () async {
      final tokensMap = await ApiService(
        'https://mainnet.archethic.net',
      ).getToken([
        '0000260d70b23efc72ce7be2a7da00a0e2eeaa6b39bd3b6e39e5a6bc600303177ef6',
      ]);
      final token =
          tokensMap['0000260d70b23efc72ce7be2a7da00a0e2eeaa6b39bd3b6e39e5a6bc600303177ef6'];

      expect(token!.collection.length, 6765);
    ...
    test('getBlockchainVersion', () async {
      ...
      expect(blockchainVersionModel.version.transaction, '4');
    });

    test('callSCFunction', () async {
      final smCallFunctionResponse = await ApiService(
        'https://testnet.archethic.net',
      ).callSCFunction(
    ...
    test('callSCFunctionWithResolveLastFalse', () async {
      final smCallFunctionResponse = await ApiService(
        ...
        jsonRPCRequest: const SCCallFunctionRequest(
          method: 'contract_fun',
          params: SCCallFunctionParams(
            contract:
  ...
  test('chainUnspentOutputs', () async {
    final chainUnspentOutputsList = await ApiService(
      'https://testnet.archethic.net',
    ).chainUnspentOutputs([
    ...
    expect(chainUnspentOutputsList.keys.first.length, greaterThan(0));

    expect(chainUnspentOutputsList.keys.last.length, greaterThan(0));

    expect(chainUnspentOutputsList.length, 2);
  });
  ...
  group('api - performance', tags: <String>[TestTags.integration], () {
    test('should handle 40 parallel calls with simulated errors', () async {
      ...
      final futures = List.generate(
        nbIteration,
        (final index) => apiService.getToken([address]),
      );
